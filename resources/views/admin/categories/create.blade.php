...
@section('page_title', 'Create Store Category')
@section('page_subtitle', 'Add a new catalog department and upload category photo')
    ...
    <form action="{{ route('admin.categories.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        ...
        <div class="bg-slate-900/80 border border-slate-800/80 rounded-3xl p-6 sm:p-8 shadow-2xl backdrop-blur-xl space-y-5">
            <div class="border-b border-slate-800 pb-4">
                <h3 class="text-base font-bold text-white">Category Information</h3>
            </div>
            ...
            <!-- Image -->
            <div>
                <label class="block text-xs font-semibold uppercase tracking-wider text-slate-400 mb-1.5">Category Thumbnail Image</label>
            ...
            <div>
                ...
                <textarea name="description" rows="3"
                          class="w-full px-4 py-3 bg-slate-950/60 border border-slate-800 rounded-xl text-sm text-white placeholder-slate-500 focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500"
                          placeholder="Brief summary of category items...">{{ old('description') }}</textarea>
            </div>
